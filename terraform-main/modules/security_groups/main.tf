...
data "terraform_remote_state" "runner_infra" {
  count = var.skip_runner_integration ? 0 : 1
...
resource "null_resource" "validate_outputs_or_fail" {
  ...
  provisioner "local-exec" {
    command = <<-EOF
      ...
      VPC_CIDR="${try(data.terraform_remote_state.runner_infra[0].outputs.vpc_cidr_block, "")}"
...
resource "aws_vpc_security_group_ingress_rule" "eks_api_from_github_runner" {
  ...
  to_port           = 443
  ip_protocol       = "tcp"
  cidr_ipv4         = try(data.terraform_remote_state.runner_infra[0].outputs.vpc_cidr_block, "10.255.255.0/24")
  description       = "PEERING: Allow Peered network - github runner- access to the cluster API"
